...
os.environ['MPLCONFIGDIR']='/tmp/dlfs-architectures/rehearsal10-final/matplotlib'
os.environ['IPYTHONDIR']='/tmp/dlfs-architectures/rehearsal10-final/ipython'
...
out=pathlib.Path('/tmp/dlfs-architectures/rehearsal10-final')
...
for i,cell in enumerate(notebook.cells):
    for output in cell.get('outputs',[]):
        if output.output_type=='stream':streams.append({'cell':i,'name':output.get('name'),'text':output.get('text','')})
        if output.output_type=='error':errors.append({'cell':i,'ename':output.get('ename'),'evalue':output.get('evalue')})
...
report={'source':str(source),'sha256':hashlib.sha256(raw).hexdigest(),'elapsed_seconds':elapsed,
    ...
    'figure_count':len(images),'figures':images,'errors':errors,'warnings':[s for s in streams if s['name']=='stderr'],
    'metrics':metrics}
