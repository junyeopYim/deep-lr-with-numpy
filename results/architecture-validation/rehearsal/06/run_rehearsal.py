...
output=Path('/tmp/dlfs-architectures/rehearsal06')
source=root/'notebooks/01_아키텍처/06_rnn.ipynb'
...
notebook.cells.append(nbformat.v4.new_markdown_cell('# 독립 실행자가 작성한 RNN 변형과 검산'))
...
for index,cell in enumerate(notebook.cells):
    ...
    for result in cell.outputs:
        if result.output_type=='stream':
            streams.append({'cell':index,'name':result.name,'text':result.text})
        if result.output_type=='error':
            errors.append({'cell':index,'data':dict(result)})
