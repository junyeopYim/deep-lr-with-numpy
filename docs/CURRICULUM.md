...
**현재 실행 자료는 00–15에 00b·01b와 2026-09-16에 더한 11권(05b·18b·23·24·25·28·32·33·34·35·35b), 2026-09-17에 더한 2권(07b SNN·10b CVNN)을 합친 총 31개 노트북입니다.** 16–63 가운데 위 11권을 뺀 나머지는 다음 제작을 위한 학습 설계입니다.
2026-09-16부터 사용자가 요청한 모델을 더합니다: LeNet은 **05b**, 물체 검출(YOLO)은 **18b**, EDM은 **35b**로 기존 번호를 유지한 채 b 번호로 끼우고(00b·01b와 같은 규칙), VAE(23–25)·GAN(28)·NCSN(32–33)·DDPM(34–35)은 아래 표의 자리 그대로 제작합니다. 11권 모두 제작을 마쳤습니다. 2026-09-17에는 같은 규칙으로 스파이킹 신경망(SNN)을 **07b**, 복소수 신경망(CVNN)을 **10b**에 끼웠습니다.
전체 71개 단위(번호 00–63의 64개 주제와 b 단위 7개)는 주제를 찾는 지도이며, 선수 개념을 확인하면서 관심 경로를 선택합니다.
...
| 07b | SNN: 막전위가 문턱을 넘을 때만 쏘는 뉴런 | 06·07·03·04·05b | 누설·적분·뺄셈 리셋 → 0/1 스파이크, rate coding, 켜진 행 더하기, 계단 forward + 대리 도함수 BPTT(리셋 경로) | 스파이크로 부호화한 실제 MNIST 분류, 부드러운 LIF gradcheck, 추론 시각 수와 덧셈 수 | [노트북](../notebooks/01_아키텍처/07b_snn.ipynb) |
...
| 10b | CVNN: 크기와 위상을 나르는 복소수 뉴런 | 00·01·03·04·10 | 곱 = 회전·확대 → 복소 선형층 = 2×2 블록 실수층, $g=\partial L/\partial x+i\,\partial L/\partial y$·`X.conj().T @ gZ`, Liouville → modReLU, 위상 등변·불변 | 마이크 8개로 소리 방향 6개 분류, 실수 MLP·크기만 MLP·빔포머와 표본 수별 비교, 위상 회전 불변 검산 | [노트북](../notebooks/01_아키텍처/10b_cvnn.ipynb) |
...
07b·10b는 연결 구조 대신 **뉴런이 나르는 값**을 바꿉니다. 07b SNN은 06·07의 상태와 BPTT에 0/1 스파이크·리셋 경로·대리 도함수를 붙이고, 10b CVNN은 10의 순열 등변성을 전체 위상 회전 $z\mapsto e^{i\varphi}z$에 대한 등변·불변으로 넓힙니다.
